export const fetchProductVariationTypeData = ({ commit }) => {
    return axios.get('/api/product-variation-type').then((response) => {
        commit('setProductVariationType', response.data.data)
    })
}

export const createtypes = ({commit }, { payload, context }) => {
    return axios.post('/api/product-variation-type', payload).then((response) => {
    }).catch((error) => {
...
export const updatProductVariationType = ({ commit }, { payload, context }) => {
    return axios.put('/api/product-variation-type/'+payload.id, payload).then((response) => {
    }).catch((error) => {
        context.errors = error.response.data.errors
   })
}
