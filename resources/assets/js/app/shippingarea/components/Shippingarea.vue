<template>
    <span>
            ...
            <div class="card-header">
                <div class="row align-items-center justify-content-between">
                    <div class="col-6 col-sm-auto d-flex align-items-center pr-0">
                        <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">Shipping areas</h5>
                    </div>
                    <div class="col-6 col-sm-auto ml-auto text-right pl-0">
                        <div id="dashboard-actions">
                            <button class="btn btn-falcon-default btn-sm" @click="newcenter" type="button">
                                <span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span>
                                <span class="d-none d-sm-inline-block ml-1">New area</span>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <table class="table table-sm mb-0 table-dashboard fs--1">
                    ...
                    <tbody id="purchases">
                        <tr v-for="dt in data" :key="dt.id" class="btn-reveal-trigger">
                            ...
                            <th class="ml-3 align-middle">{{dt.code}}</th>
        ...
        <modal name="shippingModal" classes="animated bounceInDown faster" :clickToClose="true" height="auto" :adaptive="true" :pivotY="0.2">
            <shippingareaform :base="variation" :state="state"/>
        </modal>
...
export default {
    ...
    computed: {
        ...mapGetters({
            data: 'shippingarea/getShippingarea',
            variation: 'variationtypes/getVariation',
        })
    },
    methods: {
        ...mapActions({
            fetchShippingareasData: 'shippingarea/fetchShippingareasData',
    ...
    mounted (){
        this.fetchShippingareasData()
        Bus.$on('close-variationtypesform-modal', ()=> {
            this.$modal.hide('shippingModal');
        })
        Bus.$on('new-shipping-data', (data)=> {
