<template>
    <span>
        <div class="card mb-3 animated zoomIn faster">
            <div class="card-header">
                <div class="row align-items-center justify-content-between">
                    <div class="col-6 col-sm-auto d-flex align-items-center pr-0">
                        <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">Shipping cost per area</h5>
                    </div>
                    <div class="col-6 col-sm-auto ml-auto text-right pl-0">
                        <div id="dashboard-actions">
                            <button class="btn btn-falcon-default btn-sm" @click="newcenter" type="button">
                                <span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span>
                                <span class="d-none d-sm-inline-block ml-1">New cost per area</span>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-body p-0">
                ...
                <table class="table table-sm mb-0 table-dashboard fs--1">
                    ...
                    <tr>
                        <th class="ml-3">Region</th>
                        <th class="ml-3">Method</th>
                    ...
                    <tbody id="purchases">
                        <tr v-for="dt in data" :key="dt.id" class="btn-reveal-trigger">
                            <th class="ml-3 align-middle">{{dt.countries.name}}</th>
                            <th class="ml-3 align-middle">{{dt.shippingmethods.name}}</th>
                            <th class="ml-3 align-middle">{{dt.shippingmethods.price}}</th>
                            <th class="ml-3 align-middle">{{dt.shippingmethods.description}}</th>
        ...
        <modal name="shippingCostModal" classes="animated bounceInDown faster" :clickToClose="true" height="auto" :adaptive="true" :pivotY="0.2">
            <shippingcostform :areas="areas" :costs="costs" :base="variation" :state="state"/>
        </modal>
...
export default {
    ...
    computed: {
        ...mapGetters({
            data: 'shippingcost/getShippingmethods',
            variation: 'variationtypes/getVariation',
            areas: 'shippingcost/getShippingAreas',
            costs: 'shippingcost/getShippingCosts',
        })
    },
    methods: {
        ...mapActions({
            ShippingCostData: 'shippingcost/ShippingCostData',
            selectedVariation: 'variationtypes/selectedVariation',
            fetchSerciceCountData: 'dashboard/fetchSerciceCountData',
            fetchShippingArea: 'shippingcost/fetchShippingArea',
            fetchshippingCosts: 'shippingcost/fetchshippingCosts',
        }),
        dateFormater(date){
            if (date != null && date != "") {
        ...
        newcenter(){
            ...
            this.errors = [];
            this.state = 2
            this.$modal.show('shippingCostModal')
        },
        open(cnt){
